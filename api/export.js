...
export default async function handler(req, res) {
    ...
    // 1. Ověření uživatele přes HttpOnly Cookie
    ...
    // 2. Kontrola Premium statusu (Export je jen pro Premium)
    ...
    try {
        // 3. Získání uložených scanů uživatele
        const savedRaw = await redis.lrange(`saved_scans:${email}`, 0, -1);
        ...
        savedRaw.forEach(itemStr => {
            try {
                ...
                if (typeof item === 'string') item = JSON.parse(item); // Double parse fallback

                const date = new Date(item.timestamp).toLocaleDateString();
                
                // Ošetření textu pro CSV (odstranění čárek, aby se nerozhodil formát)
